...
async function main() {
  ...
  console.log(
    `Seeded hub: ${insertedSpaces.length} spaces inserted, ${spaces.length - insertedSpaces.length} already present; ${placed} engrams placed in The Commons, ${engrams.length - missing.length} already located.`,
  );
  await pool.end();
}
