import { db, pool, expressionsTable, type InsertExpression } from "@workspace/db";
...
async function main() {
  ...
  console.log(`Seeded expressions: ${result.length} inserted, ${expressions.length - result.length} already present.`);
  await pool.end();
}
